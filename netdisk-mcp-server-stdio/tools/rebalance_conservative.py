...
# -*- coding: utf-8 -*-
"""
Conservative shard mover for /共享图集 - 保守分片策略
- 大幅降低操作频率，避免频控
- 每次只处理少量文件，长时间等待
- 适合大量文件的分片处理
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Conservative rebalance /共享图集 into shard subdirectories')
    ...
    p.add_argument('--batch-size', type=int, default=50, help='Files per batch (aggressive)')
    p.add_argument('--batch-interval', type=int, default=5, help='Seconds to wait between batches')
    p.add_argument('--limit', type=int, default=1000, help='Max files to move this run (0 for unlimited)')
...
async def perform_conservative_moves(root: str, bucket: int, max_files: int, prefix: str, 
                                  batch_size: int, batch_interval: int, limit: int, dry_run: bool) -> Dict[str, Any]:
    root = path_key(root)
    ...
    # prepare ops
    to_plan: List[Tuple[str, str, str]] = []
    ...
    i = 0
    while i < len(to_plan):
        batch = to_plan[i:i+batch_size]
        ops = [{'path': src, 'dest': dest_dir} for src, dest_dir, _ in batch]
        
        print(f"Processing batch {i//batch_size + 1}/{(len(to_plan)-1)//batch_size + 1} ({len(batch)} files)...")
        
        # single attempt with conservative approach
        try:
            ...
            print(f"✓ Batch successful: moved {moved}/{len(to_plan)} files")
            
        except Exception as e:
            msg = str(e)
            print(f"✗ Batch failed: {msg}")
            if '429' in msg or '频率' in msg or 'rate' in msg:
                print(f"Rate limited, waiting {batch_interval}s...")
        ...
        # conservative interval between batches
        if i < len(to_plan):
...
def main():
    args = parse_args()
    init_sync_db()
    res = asyncio.run(perform_conservative_moves(
        root=args.root,
